...
#uncomment to see the missclassified plot... change above to see the correct classified plot
#ax.scatter(wrong_type3, wrong_type2, wrong_type1, c='g', marker='o', s=150)
...
for testsets in range(len(myclones_data)):
    if ( (myclones_data[testsets, 11] == myclones_data[testsets, 12]) or (myclones_data[testsets, 11] == myclones_data[testsets, 17]) or (myclones_data[testsets, 12] == myclones_data[testsets, 17])):
        print "NA"

    else:
        testset_type1.append(myclones_data[testsets,11])
        testset_type2.append(myclones_data[testsets, 12])
        testset_type3.append(myclones_data[testsets, 17])


#ax.scatter(myclones_data[:,11], myclones_data[:,12], myclones_data[:,17], c='darkorange', marker='o', s=150)

ax.scatter(testset_type1, testset_type2, testset_type3, c='darkorange', marker='o', s=150)


"""
plt.scatter(wrong_type3, wrong_type1, c='g', marker='o', s=140)
...
ax.set_title('Plot of All Testsets', fontsize=18)
ax.set_xlabel('Type 3 Similarity',fontsize=15)
ax.set_ylabel('Type 2 Similarity',fontsize=15)
ax.set_zlabel('Type 1 Similarity',fontsize=15)
